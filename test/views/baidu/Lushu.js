...
;(function () {
    ...
    ;(function () {
        ...
        baidu.dom.getComputedStyle = function (element, key) {
            ...
            let doc = baidu.dom.getDocument(element),
                styles
        ...
        baidu.dom._styleFilter.filter = function (key, value, method) {
            let filters = baidu.dom._styleFilter
            let filter
    ...
    LuShu.prototype.stop = function () {
        ...
        for (
            let i = 0, t = this._opts.landmarkPois, len = t.length;
            i < len;
            i++
        ) {
            t[i].bShow = false
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _addMarker: function (callback) {
            ...
            let marker = new BMapGL.Marker(this.path[0])
            this._opts.icon && marker.setIcon(this._opts.icon)
            this._map.addOverlay(marker)
            marker.setAnimation(BMAP_ANIMATION_DROP)
            this._marker = marker
            // 变更
            let markerL = new BMapGL.Marker(this.path[0], { left: true })
            ...
            let markerR = new BMapGL.Marker(this.path[0], { right: true })
        ...
        _getDistance: function (pxA, pxB) {
            return Math.sqrt(
                Math.pow(pxA.x - pxB.x, 2) + Math.pow(pxA.y - pxB.y, 2)
            )
        },
        ...
        _move: function (initPos, targetPos, effect) {
            ...
            // 变更
            let mcDis = me._getDistance(init_pos, target_pos)
            let direction = null
            if (mcDis > 30037726) {
            ...
            //总的步长
            let count = Math.round(me._getDistance(init_pos, target_pos) / step)
            ...
            me._intervalFlag = setInterval(function () {
                ...
                if (currentCount >= count) {
                    ...
                    let x = effect(
                        ...
                        pos = BMapGL.Projection.convertMC2LL(
                            new BMapGL.Point(x, y)
                        )
                    if (pos.lng > 180) {
                        pos.lng = pos.lng - 360
                    }
        ...
        setRotation: function (prePos, curPos, targetPos, direction) {
            ...
            if (targetPos.x != curPos.x) {
                ...
                //degree  correction;
                if (
                    (!direction && targetPos.x < curPos.x) ||
                    direction === 'left'
                ) {
                    deg = -deg + 90 + 90
                } else {
        ...
        _setInfoWin: function (pos) {
            ...
            let index = me._troughPointIndex(pos)
            if (index != -1) {
                clearInterval(me._intervalFlag)
    ...
    function getGeodesicPath(points) {
        ...
        for (let i = 0; i < points.length - 1; i++) {
            let great = calcGreatCirclePath(points[i], points[i + 1])
    ...
    function calcGreatCirclePath(latLng1, latLng2) {
        ...
        let count = Math.round(distance / 150000)
        let angularDistance = calcAngularDistance(latLng1, latLng2)
        result.push(latLng1)

        for (let i = 0; i < count; i++) {
            let eachLatLng = calcMiddlePoint(
                latLng1,
                latLng2,
                i / count,
                angularDistance
    ...
    function calcMiddlePoint(latLng1, latLng2, f, delta) {
        ...
        let phi1 = toRadian(lat1)
        let phi2 = toRadian(lat2)
        let lambda1 = toRadian(lon1)
        let lambda2 = toRadian(lon2)
        ...
        let phi = Math.atan2(z, Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)))
    ...
    function calcAngularDistance(latLng1, latLng2) {
        ...
        let lat1 = toRadian(latLng1.lat)
        let lat2 = toRadian(latLng2.lat)
        let lng1 = toRadian(latLng1.lng)
        let lng2 = toRadian(latLng2.lng)
        return Math.acos(
            Math.sin(lat1) * Math.sin(lat2) +
                Math.cos(lat1) *
                    Math.cos(lat2) *
                    Math.cos(Math.abs(lng2 - lng1))
        )
    }
    ...
    CustomOverlay.prototype.initialize = function (map) {
        let div = (this._div = baidu.dom.create('div', {
    ...
    baidu.object.extend(CustomOverlay.prototype, {
        ...
        setPosition: function (poi, markerSize) {
            ...
            let px = this._map.pointToOverlayPixel(poi)
            let styleW = baidu.dom.getStyle(this._div, 'width')
            let styleH = baidu.dom.getStyle(this._div, 'height')
            let overlayW = parseInt(this._div.clientWidth || styleW, 10)
            let overlayH = parseInt(this._div.clientHeight || styleH, 10)
